# 💉 API Gerenciamento Familiar de Vacinação

API RESTful desenvolvida em Java com Spring Boot para gerenciamento de vacinação.
Atividade desenvolvida durante o Programa 1000Devs - Instituição de Ensino mesttra. - Programa de Capacitação de Desenvolvedores Juniores.

O sistema permite o controle de pacientes, vacinas, doses e imunizações, além de fornecer estatísticas relevantes sobre cobertura vacinal.
...
O projeto segue o padrão **DAO (Data Access Object)**, com separação em camadas:
...
````
<img width="977" height="528" alt="Captura de tela 2026-03-19 003158" src="https://github.com/user-attachments/assets/62588864-b55f-4942-8350-1c6c8bfb0c4b" />
---
...
- Total por vacina
- Vacinas atrasadas por paciente
- Próximas vacinas
- Ranking de vacinas mais aplicadas
...
GRANT ALL PRIVILEGES ON vacinacao.* TO 'dev'@'localhost';
FLUSH PRIVILEGES;
```
...
```json
{
  ...
  "cpf_paciente": "12345678912",
  "sexo": "M",
  "data_nascimento": "1998-06-31"
}
...
```json
{
  ...
  "profissional_aplicador": "Enfermeira Ana"
}
```

---

## 📊 Endpoints de Estatística

| Endpoint                    | Descrição             |
| --------------------------- | --------------------- |
| `/estatisticas/imunizacoes/paciente/{id_paciente}`       | Quantidade de Vacinas Aplicadas em Paciente, acesso por ID  |
| `/estatisticas/proximas_imunizacoes/paciente/{id_paciente}` | Quantidade de Vacinas Aplicáveis No Próximo Mês Para um Paciente, acesso por ID    |
| `/estatisticas/imunizacoes_atrasadas/paciente/{id_paciente}`    | Quantidade de Vacinas Atrasadas de Um Paciente, acesso por ID      |
| `/estatisticas/imunizacoes/idade_maior/{meses}` | Quantidade de Vacinas Existentes Recomendadas Acima de Idade N (sendo N em meses) |
...
```
GET /estatisticas/imunizacoes_atrasadas/paciente/12
```
...
* Não utilização de ORM (Hibernate)
* Uso de JDBC puro para maior controle
* Código explícito e previsível (ideal para aprendizado)

---

## 🧠 Aprendizados

* Arquitetura em camadas (Controller / Service / Model / DAO)
* Manipulação de banco com JDBC
* Construção de APIs REST
* Escrita de queries SQL complexas
